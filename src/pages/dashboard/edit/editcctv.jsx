...
const EditCCTV = () => {
  const { id } = useParams(); // Get the CCTV ID from the URL
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      setSuccess(`CCTV berhasil diupdate: ${data.nama}`);
      setFormData({ nama: "", lokasi: "", kode: "" }); // Reset form
      navigate("/dashboard/cctv"); // Navigate back to the CCTV list after success
    } catch (err) {
      setError(err.message);
    }
  };
